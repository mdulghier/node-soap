...
function padOffset(offset) {
	if(offset < 0) {
		offset = offset * -1;
		return '-' + ('00' + offset).slice(-2);
	}
	return '+' + ('00' + offset).slice(-2);
}

function toSoapDate(obj) {
	var tzOffset = (obj.getTimezoneOffset() / -60);
	tzOffset = 2; // hack!
	return obj.getUTCFullYear() + 
		'-' + pad(obj.getUTCMonth() + 1) +
		'-' + pad(obj.getUTCDate() + 1) + 
		padOffset(tzOffset) + ':00';
}
  ...
  p.ontext = function(text) {
    ...
    if (name === 'int' || name === 'integer') {
      ...
      value = new Date(text);
  	  ...
  	  var parseableText = text.replace(/(\d\d\d\d\-\d\d\-\d\d)(\+)/, '$1 00:00:00+');
  	  value = new Date(parseableText);
  	} else {
